...
export function astar(buf: number[][], width: number, height: number, x0: number, y0: number, x1: number, y1: number, walkFunction: WalkFunction) {
    ...
    while (frontier.length !== 0) {
        ...
        for (const next of getConnected(width, height, current.x, current.y)) {
            ...
            if (isTraversable(buf[next.y][next.x]) && (score[next.hash] === undefined || nextScore < score[next.hash])) {
...
function getConnected(width: number, height: number, x: number, y: number): Vec2WithPriority[] {
